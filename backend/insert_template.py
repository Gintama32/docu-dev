...
def insert_template(filename, name, is_default=False, update_if_exists=True):
    ...
    try:
        ...
        if existing_template:
            ...
            print(f"✅ Created new template: {new_template.name} (ID: {new_template.id}){default_text}")
            return True
...
def list_templates():
    ...
    try:
        ...
        for template in templates:
            ...
            print(f"      Created: {template.created_at.strftime('%Y-%m-%d %H:%M')} {size_info}")
        print("-" * 60)
...
Examples:
  ...
  python insert_template.py --list-files
    ...
    parser.add_argument("filename", nargs="?", help="Template filename (e.g., resume_template.html)")
    ...
    parser.add_argument("--list", action="store_true", help="List all templates in database")
    parser.add_argument("--list-files", action="store_true", help="List available template files")
    parser.add_argument("--force", action="store_true", help="Force update if template exists")
