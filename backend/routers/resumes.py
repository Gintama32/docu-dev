...
def list_resumes(
    ...
    if proposal_id:
        query = query.filter(models.Resume.project_proposal_id == proposal_id)
    if user_profile_id:
        query = query.filter(models.Resume.user_profile_id == user_profile_id)

    # Exclude soft-deleted resumes
    query = query.filter(models.Resume.deleted_at.is_(None))
    ...
    for resume in resumes:
        resume_data = {
            ...
            "experience_count": len(resume.resume_experience_details),
            "proposal_name": (resume.project_proposal.name if resume.project_proposal else None),
            "client_name": (
                resume.project_proposal.client.client_name
                if resume.project_proposal and resume.project_proposal.client
                else None
            ),
        }
        ...
        if red.experience:
            detail = {
                ...
                "experience": {
                    ...
                    "date_completed": (
                        red.experience.date_completed.isoformat() if red.experience.date_completed else None
                    ),
...
def create_resume(resume: schemas.ResumeCreate, db: Session = Depends(get_db)):
    # Get the proposal details if provided
    proposal = None
...
def update_resume(resume_id: int, resume_update: schemas.ResumeUpdate, db: Session = Depends(get_db)):
...
async def rewrite_experience_for_proposal(request: ExperienceRewriteRequest, db: Session = Depends(get_db)):
    ...
    resume_experience = next(
        (red for red in resume.resume_experience_details if red.experience_id == request.experience_id),
        None,
    )
    ...
    result = await ai_service.rewrite_experience(request.original_description, request.proposal_context)
...
async def single_ai_rewrite_experience(request: SingleAIRewriteRequest, db: Session = Depends(get_db)):
    ...
    # Use AI service to rewrite
    result = await ai_service.rewrite_experience(experience.project_description, proposal.context or proposal.name)
