...
class ResumeVariableResolver:
    ...
    def _serialize_profile(self, profile: models.UserProfile) -> Dict[str, Any]:
        ...
        return {
            ...
            # Image URL
            "main_image_url": f"http://localhost:8001/api/media/{profile.main_image_id}/raw" if profile.main_image_id else None,
            ...
            "display_name": profile.full_name or f"{profile.first_name} {profile.last_name}".strip(),
            "has_contact_info": bool(profile.email or profile.mobile or profile.address),
    ...
    def _serialize_experiences(self, experiences: List[models.Experience]) -> List[Dict[str, Any]]:
        ...
        for exp in experiences:
            ...
            if exp.client_id:
                ...
                if client:
                    client_info = {
                        ...
                        "main_contact": client.main_contact.contact_name if client.main_contact else None
                    }
            
            # Format dates
            date_started = exp.date_started.strftime("%B %Y") if exp.date_started else None
            date_completed = exp.date_completed.strftime("%B %Y") if exp.date_completed else "Present"
            date_range = f"{date_started or 'N/A'} - {date_completed}" if date_started else None
            ...
            serialized.append({
                ...
                "project_description": getattr(exp, '_display_description', exp.project_description),
                ...
                "duration_months": self._calculate_duration_months(exp.date_started, exp.date_completed)
            })
    ...
    def _apply_overrides(self, variables: Dict[str, Any], overrides: Dict[str, Any]) -> Dict[str, Any]:
